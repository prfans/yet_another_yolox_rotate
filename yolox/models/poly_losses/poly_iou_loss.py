...
from .obb_utils import (get_bbox_type, bbox2type)
...
def poly_iou_loss(pred, target, mode='linear ', eps=1e-6):
    ...
    pred_pts = pred.reshape(pred.shape[0], -1, 2)
    target_pts = target.reshape(target.shape[0], -1, 2)
...
def poly_iou_overlaps(pred_, target_, eps=1e-6):
